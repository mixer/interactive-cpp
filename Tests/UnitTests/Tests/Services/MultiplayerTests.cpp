...
MultiplayerSessionResponseTestData GetDefaultMultiplayerSessionResponseTestData(unsigned int numHostCandidates, bool isLargeSession)
{
    ...
    //        "timeouts": 
    //        {
    //            "reserved": 10000,  // Default is 10 seconds. Member is removed from the session.
    //            "inactive": 7200000,  // Default is two hours. Member is removed from the session.
    //            "ready": 60000,  // Default is one minute. Member becomes inactive.
    //            "sessionEmpty": 0  // Default is zero. Session is deleted.
    //        },
    //        "metrics": 
    //        {
    //            "latency": true,
...
public:
    ...
    void VerifyMultiplayerManagedInitialization(MultiplayerManagedInitialization^ result, web::json::value resultToVerify)
    {
        ...
        VERIFY_ARE_EQUAL(result->AutoEvaluate, resultToVerify[L"autoEvaluate"].as_bool());
    ...
    void VerifyMultiplayerSessionMember(MultiplayerSessionMember^ result, web::json::value resultToVerify)
    {
        ...
        if (!rolesJson.is_null())
        {
            ...
            {
                VERIFY_ARE_EQUAL(result->RoleType, ref new Platform::String(role.first.c_str()));
                VERIFY_ARE_EQUAL(result->RoleName, ref new Platform::String(role.second.as_string().c_str()));
                break;
            }
        }
    ...
    void VerifyMultiplayerSessionConstants(MultiplayerSessionConstants^ result, web::json::value resultToVerify)
    {
        ...
        web::json::value systemTimeoutsJson = systemJson[L"timeouts"];
        VERIFY_ARE_EQUAL_TIMESPAN_TO_MILLISECONDS(
            result->MemberReservationTimeout,
            systemTimeoutsJson[_T("reserved")].as_number().to_uint64()
            );
        VERIFY_ARE_EQUAL_TIMESPAN_TO_MILLISECONDS(
            result->MemberInactiveTimeout,
            systemTimeoutsJson[_T("inactive")].as_number().to_uint64()
            );
        VERIFY_ARE_EQUAL_TIMESPAN_TO_MILLISECONDS(
            result->MemberReadyTimeout,
            systemTimeoutsJson[_T("ready")].as_number().to_uint64()
            );
        VERIFY_ARE_EQUAL_TIMESPAN_TO_MILLISECONDS(
            result->SessionEmptyTimeout,
            systemTimeoutsJson[_T("sessionEmpty")].as_number().to_uint64()
            );
        ...
        VerifyMultiplayerManagedInitialization(result->ManagedInitialization, systemJson[L"managedInitialization"]);
...
#pragma warning(suppress: 6387)
    ...
    DEFINE_TEST_CASE(TestCreateSessionAsync)
    {
        ...
        session->SetCurrentUserRoleInfo(L"lfg", L"friend");
