...
#pragma warning(suppress: 6387)
    ...
    */
    ...
    void JoinGameHelper(Microsoft::Xbox::Services::XboxLiveContext^ xboxLiveContext)
    {
        std::unordered_map<string_t, std::shared_ptr<HttpResponseStruct>> responses;
...
#pragma warning(suppress: 4973)
        ...
        while (!isDone)
        {
            ...
            for (auto ev : events)
            {
                if (ev->EventType == MultiplayerEventType::UserAdded)
                {
                    VERIFY_IS_TRUE(ev->ErrorCode == WEB_E_INVALID_JSON_STRING);
                    isDone = true;
                }
            }
        }
    ...
    */
    ...
    void FindMatchWithQoSHelper(MatchCallingPatternType callingPattern)
    {
        ...
        bool matchFound = false, searchingTapTriggered = false, foundTapTriggered = false, waitingForClientsToJoinTapTriggered = false, waitingForClientsToUploadQoSTapTriggered = false;
        while (!matchFound)
        {
            auto events = mpInstance->DoWork();
            if (mpInstance->MatchStatus == MatchStatus::Searching && !searchingTapTriggered)
