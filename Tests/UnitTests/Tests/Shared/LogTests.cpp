...
public:
    ...
    DEFINE_TEST_CASE(WriteLog)
    {
        ...
        auto test_output = std::make_shared<test_log_output>();

        testLogger->add_log_output(test_output);

        char testLogText[] = "testlog";

        testLogger->add_log(log_entry(log_level::debug, "test", testLogText));
        testLogger->add_log(log_entry(log_level::debug, "test") << testLogText << 1);
        ...
        testLogger->add_log(log_entry(log_level::debug, "test") << testLogText << testLogText);
        testLogger->add_log(log_entry(log_level::debug, "test") << testLogText << veryverylong);

        VERIFY_ARE_EQUAL_INT(4, test_output->log_output.size());
        VERIFY_IS_TRUE(StringCompareLastCharactors(test_output->log_output[0], testLogText));
        VERIFY_IS_TRUE(StringCompareLastCharactors(test_output->log_output[1], std::string(testLogText) + "1"));
        VERIFY_IS_TRUE(StringCompareLastCharactors(test_output->log_output[2], std::string(testLogText) + testLogText));
        VERIFY_IS_TRUE(StringCompareLastCharactors(test_output->log_output[3], std::string(testLogText) + veryverylong));
    ...
    DEFINE_TEST_CASE(WriteLogStream)
    {
        ...
        *testLogger += log_entry(log_level::debug, "test") << testLogText;
        *testLogger += log_entry(log_level::debug, "test") << testLogText << 1;

        VERIFY_ARE_EQUAL_INT(2, test_output->log_output.size());
        VERIFY_IS_TRUE(StringCompareLastCharactors(test_output->log_output[0], testLogText));
        VERIFY_IS_TRUE(StringCompareLastCharactors(test_output->log_output[1], std::string(testLogText) + "1"));
    ...
    DEFINE_TEST_CASE(WriteLogConcurrent)
    {
        ...
        for (int i = 0; i < loopCount; i++)
        {
            auto task = create_task([loopCount, testLogger]()
            {
                ...
                testLogger->add_log(log_entry(log_level::debug, "test", "a"));
            });
        ...
        VERIFY_ARE_EQUAL_INT(loopCount*loopCount, test_output->log_output.size());
    }
    ...
    DEFINE_TEST_CASE(LogLevel)
    {
        ...
        VERIFY_IS_FALSE(testLogger->log_level_enabled(log_level::error));
        VERIFY_IS_FALSE(testLogger->log_level_enabled(log_level::warn));
        VERIFY_IS_FALSE(testLogger->log_level_enabled(log_level::info));
        VERIFY_IS_FALSE(testLogger->log_level_enabled(log_level::debug));

        testLogger->set_log_level(log_level::error);
        VERIFY_IS_TRUE(testLogger->log_level_enabled(log_level::error));
        VERIFY_IS_FALSE(testLogger->log_level_enabled(log_level::warn));
        VERIFY_IS_FALSE(testLogger->log_level_enabled(log_level::info));
        VERIFY_IS_FALSE(testLogger->log_level_enabled(log_level::debug));
        ...
        VERIFY_IS_TRUE(testLogger->log_level_enabled(log_level::error));
        VERIFY_IS_TRUE(testLogger->log_level_enabled(log_level::warn));
        VERIFY_IS_FALSE(testLogger->log_level_enabled(log_level::info));
        VERIFY_IS_FALSE(testLogger->log_level_enabled(log_level::debug));
        ...
        VERIFY_IS_TRUE(testLogger->log_level_enabled(log_level::error));
        VERIFY_IS_TRUE(testLogger->log_level_enabled(log_level::warn));
        VERIFY_IS_TRUE(testLogger->log_level_enabled(log_level::info));
        VERIFY_IS_FALSE(testLogger->log_level_enabled(log_level::debug));
        ...
        VERIFY_IS_TRUE(testLogger->log_level_enabled(log_level::warn));
        VERIFY_IS_TRUE(testLogger->log_level_enabled(log_level::info));
        VERIFY_IS_TRUE(testLogger->log_level_enabled(log_level::debug));
    ...
    DEFINE_TEST_CASE(LogMacro)
    {
        ...
        VERIFY_ARE_EQUAL_INT(5, test_output->log_output.size());
        VERIFY_IS_TRUE(StringCompareLastCharactors(test_output->log_output[0], "warn"));
        VERIFY_IS_TRUE(StringCompareLastCharactors(test_output->log_output[1], "error"));
        VERIFY_IS_TRUE(StringCompareLastCharactors(test_output->log_output[2], "error"));
        VERIFY_IS_TRUE(StringCompareLastCharactors(test_output->log_output[3], "errorerror21"));
        VERIFY_IS_TRUE(StringCompareLastCharactors(test_output->log_output[4], "errorerror0xf0"));
    }
