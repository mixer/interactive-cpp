...
public:
	...
	interactive_group_impl(string_t groupId = _XPLATSTR("default"));
...
public:
	...
	std::shared_ptr<interactive_group> group(_In_ const string_t& group_id = _XPLATSTR("default"));

	std::shared_ptr<interactive_scene> scene(_In_ const string_t& sceneId);
...
private:
	...
	//
	std::shared_ptr<MICROSOFT_MIXER_NAMESPACE::interactive_event> try_set_current_scene(_In_ const string_t& scene_id, _In_ const string_t& group_id = _XPLATSTR("default"));
	...
	pplx::task<void> m_processingTask;
